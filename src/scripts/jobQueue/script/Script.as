...
/******************** private functions ********************/
		...
		private function handleCityResponseEvent(r:CityStateResponseEvent) : void
		{
			...
			{
				this.logMessage(r.response.ok + ": " + r.response.msg);
			}
			else if (r.response is String)
			{
				this.logMessage(r.response + " " + r.message)
			}
		}
		...
		private function handleScriptError(err:ScriptError) : Boolean
		{
			...
			switch (err.errorID)
			{
				...
				case ErrorCode.TOO_MANY_ARMY_FOR_TRAINING_FIELD:
					m_city.currentAttacks.addItem("foo");
					logMessage("Error occured when attempting to deploy army");
					runAgain = true;
					handled = true;
					complete = false;
					break;
